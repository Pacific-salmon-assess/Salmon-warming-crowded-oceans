...
bbay_lh <- dplyr::filter(s.brood.use, Region == "Bristol Bay", DetailFlag ==1) #Only keep BBay years with detail

bbay_lh2 <-bbay_lh %>% dplyr::mutate( #new columns with age@ entry props 
                         prop_age0 = rowSums(select(., starts_with("R0")), na.rm=T)/TotalRecruits, 
