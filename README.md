...
## Files
- `make.R`: this script calls all the scripts needed to recreate the analysis. scripts should be run in the order presented in this file.

- `load.R`: load packages and scripts necessary for analysis. 

- `functions.R`: all functions written for the analysis are placed in this file, not the order below.
...
- `interaction-fits.R`: fit stationary models with an interaction term.
- `era_hbm_fit.R`: fit 'era' hierarchical bayesian models.
- `rw_model_fit.R`: fit dynamic (random walk) models.

### Generate figures and outputs

- `stat_hbm_inf.R`:  generate tables and figures from stationary models.
- `interaction-plots.R`: generate tables and figures for models with an interaction term.
- `era_hbm_inf.R`: generate figures from era models.
- `era_rw_fig.R`: make figures that combine era and RW model outputs
