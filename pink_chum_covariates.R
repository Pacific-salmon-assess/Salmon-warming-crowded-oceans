...
## Add de-trended north pacific competitors
...
np.comp.sec.pink <- dplyr::left_join(bt.complete.pink[,c("Stock.ID", "BY", "Species")], raw.comp[, c("all_spp_numbers_np", "comp_detrend_np","BY")], by= "BY")


## Merge datasets
master.pink <- dplyr::left_join(early.sst.pink, np.comp.sec.pink, by=c("BY","Stock.ID", "Species"))
master.pink.bt_w_cov1 <- dplyr::rename(master.pink, early_sst = sst_anomaly, np_all_spp_sec = all_spp_numbers_np, det_np_all_spp_sec = comp_detrend_np)
...
## competitor index: competitors in BY+2
## competitors in second year of marine life
raw.comp$BY <- raw.comp$Year - 2
np.comp.sec.chum <- dplyr::left_join(bt.complete.chum[,c("Stock.ID", "BY", "Species")], raw.comp[, c("all_spp_numbers_np", "comp_detrend_np", "BY")], by= "BY")

## Merge datasets
master.chum <- dplyr::left_join(early.sst.chum, np.comp.sec.chum, by=c("BY","Stock.ID", "Species"))
master.chum.bt_w_cov1 <- dplyr::rename(master.chum, early_sst = sst_anomaly, np_all_spp_sec = all_spp_numbers_np, det_np_all_spp_sec = comp_detrend_np)
