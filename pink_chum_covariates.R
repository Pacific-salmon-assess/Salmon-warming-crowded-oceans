...
np.pink.sec.pink <- dplyr::left_join(bt.complete.pink[,c("Stock.ID", "BY", "Species")], raw.comp[, c("all_spp_numbers_np", "pink_numbers_np", "pink_detrend_np","BY")], by= "BY")


## Merge datasets
master.pink <- dplyr::left_join(early.sst.pink, np.pink.sec.pink, by=c("BY","Stock.ID", "Species"))
master.pink.bt_w_cov1 <- dplyr::rename(master.pink, early_sst = sst_anomaly, np_pinks_sec = pink_numbers_np, np_all_spp_sec = all_spp_numbers_np, det_np_pinks_sec = pink_detrend_np)
...
raw.comp$BY <- raw.comp$Year - 2
np.pink.sec.chum <- dplyr::left_join(bt.complete.chum[,c("Stock.ID", "BY", "Species")], raw.comp[, c("pink_numbers_np", "pink_detrend_np", "BY")], by= "BY")

## Merge datasets
master.chum <- dplyr::left_join(early.sst.chum, np.pink.sec.chum, by=c("BY","Stock.ID", "Species"))
master.chum.bt_w_cov1 <- dplyr::rename(master.chum, early_sst = sst_anomaly, np_pinks_sec = pink_numbers_np, det_np_pinks_sec = pink_detrend_np)
