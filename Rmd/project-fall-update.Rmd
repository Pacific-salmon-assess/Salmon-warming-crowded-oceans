...
2.  A-priori change point (or "Era") models that allow relationships to vary among periods that corresponded with observed Northeast Pacific regime shifts in 1976/77 and 1988/89, as well as the onset of the recent marine heat wave period in 2014 (e.g., [Malick 2020](https://onlinelibrary.wiley.com/doi/10.1111/fog.12469))
...
ggplot(df.dot) +
  ...
  labs(x = "Posterior covariate effects",
       y = "",
       color = "",
       shape = "") +
  facet_wrap( ~ var) +
...
ggplot(df.dyn.reg.2c) +
  ...
  scale_y_continuous(limits=c(-.75,.75), breaks=c(-0.5, 0, 0.5)) + 
  scale_colour_manual(values=col.region, aesthetics=c("colour", "fill")) +
  theme_sleek() + theme(legend.position="none") +
   labs(x="Brood Year", y="Posterior mean covariate effects")
