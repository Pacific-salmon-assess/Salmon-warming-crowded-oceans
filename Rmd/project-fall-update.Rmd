...
The last class of non-stationary models estimates discrete covariate effects $\gamma_{k,i,q}$ dictated by regime state $q$. The estimated timeseries of hidden regime states $z_{t}$ is modelled as a Markov process (ie. year-to-year regime states depend only on the state in the previous time-step ($p(z_{t}|z_{t-1})$)) that progresses according to an estimated transition probability matrix. The model is evaluated based on the joint likelihood of the observations given each regime $p(y_{1:T}|z_{1:T})$ and the regime sequence $p(z_{1:T})$.

The hidden Markov model is conceptually similar to the random walk, but where the coefficient effects are pooled into discrete ordered distributions (i.e. low or high $\gamma$) and the probability of being in each state is evaluated at each time-step. The sequence of most probable regime states through time is therefore inferred from the data rather than set a priori as in the era model.
...
There is strong evidence for latitudinal responses to both ocean warming and interspecific competition, revealed by the stationary models (Figure 3). The effect of SST ranges from weakly negative in the southernmost region to strongly positive in the northernmost region, where the magnitude of the SST is double what it is in the south. Conversely, competitor abundance has a strongly negative effect in the south but a weak effect in the north (Figure 3).

### Evidence for nonstationarity
The various classes of nonstationary models imply variably strengthening, weakening, or emerging relationships over time and across regions. For example, assuming predefined ocean regimes in the Era model resulted in a negative, slightly shifting SST relationship in the West Coast region, while the Random Walk model suggests a gradually declining relationship from slightly positive in the earliest years. Across Alaska regions, the era model predicts that the 3 northern regions similarly shift from weakly positive to slightly negative, but the random walk model predicts relatively stationary relationships over time in Southeast Alaska and the Gulf of Alaska. 

The Southeast Alaska region may exhibit unique relationships due to its ocean conditions being highly dependent on year-to-year shifts in the North Pacific current (?). While the era model predicts weak effects with SST in Southeast Alaska across time periods, the random walk model suggests directional change that is dramatically negative in the last two decades. 

Some common insights arise from the era and random walk models, including: 
- A strong negative shift in response to competitor abundance in the Bering Sea, where Pink salmon abundance more than doubles over the time period examined,
...
**Figure 3**. Posterior probability distributions of the predicted effect of SST (top), competitors (middle), and the combined effect (bottom) from all covariate terms, on sockeye salmon survival. Regional hyperdistributions of the covariate effects are in bold lines, with individual stock-specific distributions illustrated by the light lines. Covariate effects are standardized (i.e., per standard deviation unit increase in each covariate).
...
```
**Figure 5**. Posterior probability distributions of the predicted effect of SST and competitors on Sockeye productivity over three pre-defined time periods/eras (earliest in top panel). Breakpoints between eras correspond with documented NP ocean regime shifts. Regional mean (hyperdistribution) covariate effects are in bold lines and individual stocks' distributions are in light lines. 
...
- Add a 'marine heatwave' time period to era models that begins in ~2014 at the onset of several anomolously warm years at sea
