...
map <- ggplot() + 
      ...
      ggspatial::geom_spatial_point(data = sock.info,
                         aes(x=lon, y=lat, col=ocean_label2, shape=ocean_label2, fill=ocean_label2), 
      ...
      scale_colour_manual(values=col.region) + 
      scale_fill_manual(values=col.dk) + 
...
covar.dat <- sock %>% select(Stock, Ocean.Region2, early_sst_stnd, np_pinks_sec_stnd) %>% pivot_longer(cols=c(early_sst_stnd, np_pinks_sec_stnd), names_to = "covar") %>% mutate(covar_nam = ifelse(covar=="early_sst_stnd", "SST", "Comp"))
...
In the simplest stationary model we model salmon productivity hierarchically as a function of spawner abundance, early marine ocean conditions, and competitor abundance:
...
\end{equation}
where $w$ is process variation and $\sigma_{w}^2$ determines the degree if temporal variability in the $\gamma$ series. For these models the covariate effects were not modelled independently instead of being exchangeable among populations within regions(i.e., hierarchically). 


## Hidden Markov models
The last class of non-statioanry models we considered were the most flexible and...
...
```{r stationary models: posterior probability dist.}
...
FIGURE: "sparkline plots" (like above but with time series of posteriors, stocks as rows)
 -- Decided not to include sparkline plots -- 

```{r random walk models: figures}
#Data
...
ggplot(df.dyn.reg.2c) +
  ...
  facet_grid(cols=vars(ocean_region_lab), rows=vars(varnam), scales="free_y") + 
  ylim(c(-1,1)) + 
