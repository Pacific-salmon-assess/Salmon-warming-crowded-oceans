...
for(n in 1:length(fit)) {
  ...
  write.csv(tab.coef, file = paste0("./pub/model_coefficients_", fitnam[[n]],  ".csv"))
  ...
  write.csv(stock.tbl, file = paste0("./pub/si_main_model_stock_params_", fitnam[[n]],  ".csv"))
  ...
  pdf(paste0("./pub/fig_post_density_", fitnam[[n]], ".pdf"), width = 4, height = 6)
      print(g)
  dev.off()
  ...
  pdf(paste0("./pub/Sfig_coef_dot_main_", fitnam[[n]], ".pdf"), width = 6.5, height = 6.0)
      print(g)
  dev.off()
...
## Table: coefficients ----
  ...
  write.csv(tab.coef, file = paste0("./pub/model_coefficients_", fitnam.oc[[n]],  ".csv"))
  
  
  
  ## Fig: Posterior percent change density ------------------- 
  # colours currently not working
  ...
  g <- ggplot(m.df) +
    ...
    geom_path(aes(x = x, y = y, color = region), linewidth = 1, alpha=1, 
              na.rm = TRUE) +
    scale_color_manual(values = rev(col.stock)) +
    labs(x = "Percent change in R/S",
         y = "Posterior density",
         color = "") +
    scale_x_continuous(limits = c(-50, 50), expand = c(0, 0)) +
  ...
  pdf(paste0("./pub/fig_post_density_", fitnam.oc[[n]], ".pdf"), width = 4, height = 6)
  print(g)
  dev.off()
  ...
  pdf(paste0("./pub/Sfig_coef_dot_main_", fitnam.oc[[n]], ".pdf"), width = 6.5, height = 6.0)
  print(g)
  dev.off()
...
## Fig: Map + covars ---------------------------------------
# Currently shows 3 ocean regions - Make another with 4 
