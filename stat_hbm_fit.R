...
fig.dir <- here::here("figures", "stat", speciesFlag, "hbm_fit") # place to store figures generated in this script
fit.dir <- here::here("output", "models", "stat", speciesFlag) # place to store model fits
diag.dir <- here::here("output", "diagnostics", "stat", speciesFlag) # place to store diagnostics
...
stat_a <- rstan::stan(file = "./stan/hbm_stat_2c.stan",
                     ...
                     control = list(adapt_delta = 0.99,
                                    max_treedepth = 10))
save(stat_a, file = here::here(fit.dir, "stat_a.RData"))

## Diagnostic plots
pdf(here::here(fig.dir, "stat_a_diag.pdf"), width = 7, height = 5)
...
#plot_post_pc(stat_a, stan.dat.all$y, here::here(fig.dir, "stat_a_yrep.pdf")) # Bugging

loo.stat_a <- rstan::loo(stat_a, cores = 4)
save(loo.stat_a, file = here::here(diag.dir, "loo_stat_a.RData"))
waic.stat_a <- loo::waic(loo::extract_log_lik(stat_a, "log_lik"))
save(waic.stat_a, file = here::here(diag.dir, "waic_stat_a.RData"))
pdf(here::here(fig.dir, "stat_a_loo.pdf"), width = 7, height = 5)
    plot(loo.stat_a, label_points = TRUE)
dev.off()

r2.stat_a <- bayes_R2(data_master$lnRS, as.matrix(stat_a, pars = "yhat"))
save(r2.stat_a, file = here::here(diag.dir, "r2_stat_a.Rdata"))

pdf( here::here(fig.dir, "stat_a_resid.pdf"), width = 8, height = 8)
...
stat_tr <- rstan::stan(file = "./stan/hbm_stat_2c.stan",
                      ...
                      control = list(adapt_delta = 0.95,
                                     max_treedepth = 10))
save(stat_tr, file = here::here(fit.dir, "stat_tr.RData"))

## Diagnostic plots
pdf(here::here(fig.dir, "stat_tr_diag.pdf"), width = 7, height = 5)
...
#plot_post_pc(stat_tr, stan.dat.tr$y, data = data_master[data_master$BY >= 1975,],
         #    pdf.path = here::here(fig.dir, "stat_tr_yrep.pdf"))
# Bugging 

loo.stat_tr <- rstan::loo(stat_tr, cores = 4)
save(loo.stat_tr, file = here::here(diag.dir, "loo_stat_tr.RData"))
waic.stat_tr <- loo::waic(loo::extract_log_lik(stat_tr, "log_lik"))
save(waic.stat_tr, file = here::here(diag.dir, "waic_stat_tr.RData"))
pdf(here::here(fig.dir, "stat_tr_loo.pdf"), width = 7, height = 5)
plot(loo.stat_tr, label_points = TRUE)
dev.off()

r2.stat_tr <- bayes_R2(data_master$lnRS[data_master$BY >= 1975], as.matrix(stat_tr, pars = "yhat"))
save(r2.stat_tr, file = here::here(diag.dir, "r2_stat_tr.RData"))

pdf(here::here(fig.dir, "stat_tr_resid.pdf"), width = 8, height = 8)
...
if(speciesFlag == "Sockeye") {
  ...
  stat_ctrl <- rstan::stan(file = "./stan/hbm_stat_2c.stan",
                       ...
                       control = list(adapt_delta = 0.95,
                                      max_treedepth = 10))
  save(stat_ctrl, file = here::here(fit.dir, "stat_ctrl.RData"))
  
  ## Diagnostic plots 
  pdf(here::here(fig.dir, "stat_ctrl_diag.pdf"), width = 7, height = 5)
  ...
  #plot_post_pc(stat_ctrl, stan.dat.ctrl$y, data = data_master[data_master$BY %in% c.by & data_master$Stock %in% c.stk, ],
   #            pdf.path = here::here(fig.dir, "stat_ctrl_yrep.pdf"))
  # BUgging
  
  loo.stat_ctrl <- rstan::loo(stat_ctrl, cores = 4)
  save(loo.stat_ctrl, file = here::here(diag.dir, "loo_stat_ctrl.RData"))
  waic.stat_ctrl <- loo::waic(loo::extract_log_lik(stat_ctrl, "log_lik"))
  save(waic.stat_ctrl, file = here::here(diag.dir, "waic_stat_ctrl.RData"))
  pdf(here::here(fig.dir, "stat_ctrl_loo.pdf"), width = 7, height = 5)
  ...
  #save(r2.stat_ctrl, file = here::here(diag.dir, "r2_stat_ctrl.RData"))
  
  pdf(here::here(fig.dir, "stat_ctrl_resid.pdf"), width = 8, height = 8)
