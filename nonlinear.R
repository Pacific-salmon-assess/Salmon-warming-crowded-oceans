...
raw.sst.index <- clim.wgt.avg(brood.table = sock,
                              env.data = raw.clim.sock,
                              env.covar = "sst_raw",
                              type = "first_year",
                              out.covar = "early_sst")

cov.dat <- full_join(raw.sst.index, sock[,c("Stock.ID", "Ocean.Region2","early_sst_stnd", "np_pinks_sec", "BY", "lnRS")] , by=c("Stock.ID", "BY"))
...
load(file=here('output', 'models', 'stat', 'sockeye', 'stat_nocov.RData'), verbose=T)
summ_nc <- rstan::summary(nocov, pars="yresid", probs=c(0.025, 0.5, 0.975))$summary

# Also load residuals from fit with covariates
load(file="./output/models/stat/sockeye/stat_sub.RData", verbose=T)
...
g <- ggplot(cov.dat.resid) + 
    geom_point(aes(x=BY, y=resids, col=ocean_region_lab), alpha=0.2, shape=16) + 
    hline_0(alpha=0.2) + geom_vline(xintercept=c(1989, 2011), alpha=0.2, lty="dashed") +
    geom_smooth(aes(x=BY, y=resids, col=ocean_region_lab)) +
...
png(filename=here(fig.dir, 'no_cov_residsxtime.png'), width=801*2, height=486*2, res=72*3)
print(g)
dev.off()
...
g <- ggplot(cov.dat.resid) + 
    ...
    hline_0(alpha=0.2) + geom_vline(xintercept=c(1989, 2011), alpha=0.2, lty="dashed") +
    geom_smooth(aes(x=BY, y=resids), col="gray50", alpha=0.7, se=F) +
...
g <- ggplot(cov.dat.resid) + geom_point(aes(x=early_sst, y=resids, col=ocean_region_lab), alpha=0.2, shape=16) +
      hline_0(alpha=0.2) +
      geom_smooth(aes(x=early_sst, y=resids, col=ocean_region_lab)) + 
...
g <- ggplot(cov.dat.resid) + geom_point(aes(x=early_sst, y=resids, col=ocean_region_lab), alpha=0.2, shape=16) +
      hline_0(alpha=0.2) +
      geom_smooth(aes(x=early_sst, y=resids), col="grey50") +
...
g <- ggplot(cov.dat.resid) + geom_point(aes(x=np_pinks_sec, y=resids, col=ocean_region_lab), alpha=0.2, shape=16) + 
      hline_0(alpha=0.2) +
      geom_smooth(aes(x=np_pinks_sec, y=resids, col=ocean_region_lab), alpha=0.5) +
...
ggplot(cov.dat.resid) + 
      geom_point(aes(x=early_sst, y=BY, col=resids, size=abs(resids)), alpha=0.7) + 
      theme_sleek() +
      guides(size="none") +
...
g <- cov.dat.resid %>%
  ...
  geom_point(aes(x=early_sst, y=resids, col=factor(regxera, levels=unique(regxera))), alpha=0.2, shape=16) +
  geom_smooth(aes(x=early_sst, y=resids, col=factor(regxera, levels=unique(regxera))), method="lm", se=F) +
...
g <- cov.dat.resid %>%
  ...
  mutate(regxera = paste(ocean_region_lab, era, sep=".")) %>%
  ggplot() + hline_0(col="gray70") + 
  geom_point(aes(x=np_pinks_sec, y=resids, col=factor(regxera, levels=unique(regxera))), alpha=0.2, shape=16) +
  geom_smooth(aes(x=np_pinks_sec, y=resids, col=factor(regxera, levels=unique(regxera))), method="lm", se=F) +
  scale_colour_manual(values=col.eras) +
  facet_wrap(vars(ocean_region_lab), scales="free_y") +
...
# Bind with data
detrend.resid <- cbind(sock.detrend, summ_dt[,"mean"])
...
# Bind with data
detrend.resid.era <- cbind(sock.detrend, summ_era_dt[,"mean"])
