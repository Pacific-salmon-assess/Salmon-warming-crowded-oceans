// Hidden Markov Model -- 2 time-varying covariates + K hidden states
// Single stock
...
parameters {
  ...
  vector[K] beta2; //covariate 2 - K states
}
...
{ // Forward algorithm log p(z_t = j | y_{1:t})
 ...
 logalpha[1] = log(pi1) + normal_lpdf(R_S[1] |log_a - b*S[1] + beta1*X1[1] + beta2*X2[1], sigma);
  ...
  accumulator1[i] = logalpha[t-1, i] + log(A[i, j]) + normal_lpdf(R_S[t] |log_a - b*S[t]+ beta1[i]*X1[t] + beta2[i]*X2[t], sigma);
  }
...
generated quantities {
  ...
  accumulator2[i] = logbeta[t, i] + log(A[j, i]) + normal_lpdf(R_S[t] | log_a - b*S[t]+beta1[i]*X1[t] + beta2[i]*X2[t], sigma);
  }
  ...
  for (j in 1:K)
  delta[1, K] = normal_lpdf(R_S[1] | log_a - b*S[1]+beta1[j]*X1[1]+beta2[j]*X2[1], sigma);
  for (t in 2:N) {
    for (j in 1:K) { // j = current (t)
      delta[t, j] = negative_infinity();
      for (i in 1:K) { // i = previous (t-1)
        real logp;
        logp = delta[t-1, i] + log(A[i, j]) + normal_lpdf(R_S[t] | log_a - b*S[t]+beta1[j]*X1[t]+beta2[j]*X2[t], sigma);
        if (logp > delta[t, j]) {
          bpointer[t, j] = i;
          delta[t, j] = logp;
        }
...
for(n in 1:N)log_lik[n] = normal_lpdf(R_S[n]|log_a - S[n]*b+beta1[zstar[n]]*X1[n]+beta2[zstar[n]]*X2, sigma);
