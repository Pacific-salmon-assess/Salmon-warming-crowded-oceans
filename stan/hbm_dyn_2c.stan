...
//      - gammas are series-specific
//      - sigma_gamma is series specific
//   kappa : time-varying k[t] = k[t-1] + N(0, sigma_kappa)
//      - non-centered parameterization
//      - kappas are series-specific
//      - sigma_kappa is series specific
...
data {
    ...
    real sigma_kappa_df;                  // prior df for sigma_kappa
    real sigma_kappa_mu;                  // prior mu for sigma_kappa
    real sigma_kappa_sd;                  // prior sd for sigma_kappa
}
parameters {
...
transformed parameters {
    ...
    real kappa[N];                        // 3rd covariate effect
    ...
    for(i in 1:n_series) {
        ...
        for(t in (y_start[i]+1):y_end[i]) {
            ...
            kappa[t] = tmp_kappa + sigma_kappa[i] * d_kappa[t-1];
            yhat[t] = alpha[i] + beta[i] * x1[t] + gamma[t] * x2[t] + kappa[t] * x3[t] + phi * tmp_epsilon;
...
model {
    ...
    sigma_kappa ~ student_t(sigma_kappa_df, sigma_kappa_mu, sigma_kappa_sd);
...
generated quantities {
    ...
    real signal_noise_g[n_series];        // signal-to-noise ratio for gamma
    real signal_noise_k[n_series];        // signal-to-noise ratio for kappa
    ...
    for(i in 1:n_series) {
        signal_noise_g[i] = sigma_gamma[i] / sigma[i];
        signal_noise_k[i] = sigma_kappa[i] / sigma[i];
    }
}
