// Hierarchical Ricker model + AR1 error term + 3 covariates + interaction term
//
// Parameters:
...
//   lambda: exchangeable within a group -> Normal(mu_lambda, sigma_lambda)
//      - non-centered parameterization
//      - sigma_gamma shared among series within a group
//   chi: gamma x kappa interaction term -
//        exchangeable within a group -> Normal(mu_chi, sigma_chi)
//      - non-centered parameterization
//      - sigma_gamma shared among series within a group
...
parameters {
    ...
    real mu_lambda[Ng_groups];            // population-level x4 mean
    real mu_chi[Ng_groups];               // population-level interaction mean
    ...
    real d_lambda[n_series];              // random lambda deviate
    real d_chi[n_series];                 // random chi deviate
    ...
    real<lower=0> sigma_chi[Ng_groups];   // population-level interaction SD
...
transformed parameters {
    ...
    real lambda[n_series];                // series-specific x4 effect
    real chi[n_series];                   // series-specific interaction effect
    ...
    for(i in 1:n_series) {
        ...
        lambda[i] = mu_lambda[g_group[i]] + sigma_lambda[g_group[i]] * d_lambda[i];
        chi[i]   = mu_chi[g_group[i]] + sigma_chi[g_group[i]] * d_chi[i];

        // first data point in series
        yhat[y_start[i]] = alpha[i] + beta[i]*x1[y_start[i]] + gamma[i]*x2[y_start[i]] + kappa[i]*x3[y_start[i]] + lambda[i]*x4[y_start[i]] + chi[i]*(x2[y_start[i]]*x3[y_start[i]]);
        epsilon[y_start[i]] = y[y_start[i]] - yhat[y_start[i]];

        for(t in (y_start[i]+1):y_end[i]) {
            tmp_epsilon = epsilon[t-1];
            yhat[t] = alpha[i] + beta[i]*x1[t] + gamma[i]*x2[t] + kappa[i]*x3[t] + lambda[i]*x4[t] + chi[i]*(x2[t]*x3[t]) + phi * tmp_epsilon;
