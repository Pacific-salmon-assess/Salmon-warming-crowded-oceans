...
for(r in 1:R){
  if(r == 1){
    ...
    for(i in (L_c[r-1]+2):L_c[r]){
     g_dev[J_l][Lstart[i]:Lend[i]] = (diag_pre_multiply(rep_vector(sigma_g_s[r], J[r]), Lcorr_g[(J[r-1]+1):J[r]]) * z_dev_g[J_l][Lstart[i]:Lend[i]]);
     k_dev[J_l][Lstart[i]:Lend[i]] = (diag_pre_multiply(rep_vector(sigma_k_s[r], J[r]), Lcorr_k[(J[r-1]+1):J[r]]) * z_dev_k[J_l][Lstart[i]:Lend[i]]); 
    }
  }
}
