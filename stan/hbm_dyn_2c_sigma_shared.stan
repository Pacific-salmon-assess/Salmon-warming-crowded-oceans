...
data {
    ...
    int<lower=0> g_group[n_series];       // gamma (and kappa) grouping factor
    real x1[N];                           // 1st covariate, S
...
parameters {
    real mu_alpha[Na3. _groups];             // population-level intercept
...
transformed parameters {
    ...
    for(i in 1:n_series) {
        ...
        yhat[y_start[i]] = alpha[i] + beta[i] * x1[y_start[i]] + gamma[y_start[i]] * x2[y_start[i]] + kappa[y_start[i]] * x3[y_start[i]];
        ...
        for(t in (y_start[i]+1):y_end[i]) {
            ...
            gamma[t] = tmp_gamma + sigma_gamma[g_group[i]] * d_gamma[t-1];
            kappa[t] = tmp_kappa + sigma_kappa[g_group[i]] * d_kappa[t-1];
...
generated quantities {
    ...
    real signal_noise_k[n_series];          // signal-to-noise ratio
    for(i in 1:n_series) {
    ...
    for(i in 1:n_series) {
        signal_noise_g[i] = sigma_gamma[g_group[i]] / sigma[i];
        signal_noise_k[i] = sigma_kappa[g_group[i]] / sigma[i];
    }
}
