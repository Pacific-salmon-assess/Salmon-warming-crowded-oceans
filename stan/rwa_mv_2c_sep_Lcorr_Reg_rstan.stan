...
data{
  ...
  int J[R]; // number of stocks in each region
  int L[R]; // years to be estimated in each region
  int L_c[R]; // cumulative sum of L[r]
  int J_i[N]; // index of stocks in data
  int J_ii[N]; //index of stock-year combination in data (eg. stock 1-year 1, stock 1-year 2, etc)
  int J_l[Ng]; // index mapping 1:L order of Ng to 1:J order
  int Lstart[L_tot]; // start position of each year in J_l
  int Lend[L_tot]; // end position of each year in J_l
  int j_init[J_tot]; // position of first year in J_l for each stock
...
// calculate annual deviates separately for each region
  // region 1
  for(i in 2:L_c[1]){ 
    g_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_g_s[1], J[1]), Lcorr_g_1) * z_dev_g[J_l[Lstart[i]:Lend[i]]]);
    k_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_k_s[1], J[1]), Lcorr_k_1) * z_dev_k[J_l[Lstart[i]:Lend[i]]]);
    g_t[J_l[Lstart[i]:Lend[i]]] = g_t[J_l[Lstart[i-1]:Lend[i-1]]] + g_dev[J_l[Lstart[i]:Lend[i]]];
    k_t[J_l[Lstart[i]:Lend[i]]] = k_t[J_l[Lstart[i-1]:Lend[i-1]]] + k_dev[J_l[Lstart[i]:Lend[i]]];
  }
  // region 2
  for(i in (L_c[1]+2):L_c[2]){ 
    g_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_g_s[2], J[2]), Lcorr_g_2) * z_dev_g[J_l[Lstart[i]:Lend[i]]]);
    k_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_k_s[2], J[2]), Lcorr_k_2) * z_dev_k[J_l[Lstart[i]:Lend[i]]]);
    g_t[J_l[Lstart[i]:Lend[i]]] = g_t[J_l[Lstart[i-1]:Lend[i-1]]] + g_dev[J_l[Lstart[i]:Lend[i]]];
    k_t[J_l[Lstart[i]:Lend[i]]] = k_t[J_l[Lstart[i-1]:Lend[i-1]]] + k_dev[J_l[Lstart[i]:Lend[i]]];
  }
  // region 3 
  for(i in (L_c[2]+2):L_c[3]){ 
    g_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_g_s[3], J[3]), Lcorr_g_3) * z_dev_g[J_l[Lstart[i]:Lend[i]]]);
    k_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_k_s[3], J[3]), Lcorr_k_3) * z_dev_k[J_l[Lstart[i]:Lend[i]]]);
    g_t[J_l[Lstart[i]:Lend[i]]] = g_t[J_l[Lstart[i-1]:Lend[i-1]]] + g_dev[J_l[Lstart[i]:Lend[i]]];
    k_t[J_l[Lstart[i]:Lend[i]]] = k_t[J_l[Lstart[i-1]:Lend[i-1]]] + k_dev[J_l[Lstart[i]:Lend[i]]];
  }
   // region 4 
  for(i in (L_c[3]+2):L_c[4]){ 
    g_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_g_s[4], J[4]), Lcorr_g_4) * z_dev_g[J_l[Lstart[i]:Lend[i]]]);
    k_dev[J_l[Lstart[i]:Lend[i]]] = (diag_pre_multiply(rep_vector(sigma_k_s[4], J[4]), Lcorr_k_4) * z_dev_k[J_l[Lstart[i]:Lend[i]]]);
    g_t[J_l[Lstart[i]:Lend[i]]] = g_t[J_l[Lstart[i-1]:Lend[i-1]]] + g_dev[J_l[Lstart[i]:Lend[i]]];
    k_t[J_l[Lstart[i]:Lend[i]]] = k_t[J_l[Lstart[i-1]:Lend[i-1]]] + k_dev[J_l[Lstart[i]:Lend[i]]];
  }
}  
model{
  for(j in 1:J_tot)log_b[j] ~ normal(logbeta_pr[j],logbeta_pr_sig[j]); // informative priors for b, by stock
  log_a ~ normal(1.5,2); 
  g_s0 ~ normal(0,5); 
  k_s0 ~ normal(0,5);
...
// Likelihood
  R_S ~ normal(log_a[J_i] - S*b + g_t[J_ii].*X1 + k_t[J_ii].*X2, sigma[J_i]); 
...
vector[L_tot] gamma_t_m; // NOT by region yet 
