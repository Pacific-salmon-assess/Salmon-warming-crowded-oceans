...
//      - sigma_gamma shared among series within a group (shared across eras)
//      - three eras -> early (mu_gamma2); middle (mu_gamma2); late (mu_gamma3)
//   kappa: exchangeable within a group and era -> Normal(mu_gamma, sigma_gamma)
//      - non-centered parameterization
//      - mu_gamma shared among series within a group and era
//      - sigma_gamma shared among series within a group (shared across eras)
//      - three eras -> early (mu_gamma2); middle (mu_gamma2); late (mu_gamma3)
...
parameters {
    ...
    real mu_kappa1[Ng_groups];            // population-level x3 mean -- early era
    real mu_kappa2[Ng_groups];            // population-level x3 mean -- middle era
    real mu_kappa3[Ng_groups];            // population-level x3 mean -- late era
    real mu_lambda1[Ng_groups];           // population-level x4 mean -- early era
    real mu_lambda2[Ng_groups];           // population-level x4 mean -- middle era
    real mu_lambda3[Ng_groups];           // population-level x4 mean -- late era
    ...
    real d_kappa1[n_series];              // random kappa1 deviate
    real d_kappa2[n_series];              // random kappa2 deviate
    real d_kappa3[n_series];              // random kappa3 deviate
    real d_lambda1[n_series];             // random lambda1 deviate
    real d_lambda2[n_series];             // random lambda2 deviate
    real d_lambda3[n_series];             // random lambda3 deviate
    ...
    real<lower=0> sigma_lambda[Ng_groups]; // population-level x4 SD
}
transformed parameters {
    ...
    real kappa1[n_series];                // series-specific x3 effect (early)
    real kappa2[n_series];                // series-specific x3 effect (middle)
    real kappa3[n_series];                // series-specific x3 effect (late)
    real lambda1[n_series];               // series-specific x4 effect (early)
    real lambda2[n_series];               // series-specific x4 effect (middle)
    real lambda3[n_series];               // series-specific x4 effect (late)
    ...
    for(i in 1:n_series) {
        ...
        for(t in (y_start[i]):y_end[i]) {
            ...
            kappa1[i] = (mu_kappa1[g_group[i]] + sigma_kappa[g_group[i]] * d_kappa1[i]);
            kappa2[i] = (mu_kappa2[g_group[i]] + sigma_kappa[g_group[i]] * d_kappa2[i]);
            kappa3[i] = (mu_kappa3[g_group[i]] + sigma_kappa[g_group[i]] * d_kappa3[i]);
            lambda1[i] = (mu_lambda1[g_group[i]] + sigma_lambda[g_group[i]] * d_lambda1[i]);
            lambda2[i] = (mu_lambda2[g_group[i]] + sigma_lambda[g_group[i]] * d_lambda2[i]);
            lambda3[i] = (mu_lambda3[g_group[i]] + sigma_lambda[g_group[i]] * d_lambda3[i]);
            if(t == y_start[i]) { // first data point in series
                yhat[y_start[i]] = alpha[i] + beta[i] * x1[y_start[i]] +
                    ...
                    (kappa1[i] * x3[y_start[i]] * era1[y_start[i]]) +
                    (kappa2[i] * x3[y_start[i]] * era2[y_start[i]]) +
                    (kappa3[i] * x3[y_start[i]] * era3[y_start[i]]) +
                    (lambda1[i] * x4[y_start[i]] * era1[y_start[i]]) +
                    (lambda2[i] * x4[y_start[i]] * era2[y_start[i]]) +
                    (lambda3[i] * x4[y_start[i]] * era3[y_start[i]]);
                ...
                yhat[t] = alpha[i] + beta[i] * x1[t] +
                    ...
                    (kappa1[i] * x3[t] * era1[t]) +
                    (kappa2[i] * x3[t] * era2[t]) +
                    (kappa3[i] * x3[t] * era3[t]) +
                    (lambda1[i] * x4[t] * era1[t]) + 
                    (lambda2[i] * x4[t] * era2[t]) +
                    (lambda3[i] * x4[t] * era3[t]) +
                    phi * tmp_epsilon;
...
model {
    ...
    mu_kappa1 ~ normal(0, 5);
    mu_kappa2 ~ normal(0, 5);
    mu_kappa3 ~ normal(0, 5);
    sigma_kappa ~ student_t(3, 0, 3);
    mu_lambda1 ~ normal(0,5);
    mu_lambda2 ~ normal(0,5);
    mu_lambda3 ~ normal(0,5);
    sigma_lambda ~ student_t(3, 0, 3);
    ...
    d_kappa2 ~ normal(0, 1);
    d_kappa3 ~ normal(0, 1);
    d_lambda1 ~ normal(0, 1);
    d_lambda2 ~ normal(0, 1);
    d_lambda3 ~ normal(0, 1);
    beta ~ normal(0, 5);
