...
//      - non-centered parameterization                       // CHANGED: kappa is now stationary
//      - sigma_kappa shared among series within a group       // CHANGED: no era-specific structure
...
parameters {
    ...
    real mu_kappa[Ng_groups];             // population-level x3 mean            // CHANGED: single (stationary) mean, no eras
    ...
    real d_kappa[n_series];               // random kappa deviate               // CHANGED: single (stationary) deviate, no eras
...
transformed parameters {
    ...
    real kappa[n_series];                 // series-specific x3 effect          // CHANGED: single (stationary) effect, no eras
    ...
    for(i in 1:n_series) {
        ...
        kappa[i] = mu_kappa[g_group[i]] + sigma_kappa[g_group[i]] * d_kappa[i]; // CHANGED: kappa computed once per series, outside the time loop (stationary)
        ...
        for(t in (y_start[i]):y_end[i]) {
            ...
            if(t == y_start[i]) { // first data point in series
                yhat[y_start[i]] = alpha[i] + beta[i] * x1[y_start[i]] +
                    ...
                    kappa[i] * x3[y_start[i]];                                 // CHANGED: kappa applied without era switches
                ...
                yhat[t] = alpha[i] + beta[i] * x1[t] +
                    ...
                    kappa[i] * x3[t] +                                        // CHANGED: kappa applied without era switches
                    phi * tmp_epsilon;
...
model {
    ...
    mu_kappa ~ normal(0, 5);                                                   // CHANGED: single (stationary) prior, no eras
    ...
    d_gamma2 ~ normal(0, 1);
    d_gamma3 ~ normal(0, 1);
    d_kappa ~ normal(0, 1);                                                    // CHANGED: single (stationary) prior, no eras
    beta ~ normal(0, 5);
