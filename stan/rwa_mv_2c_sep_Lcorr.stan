...
//MVN parameters  
  cholesky_factor_corr[J] Lcorr_g;
  cholesky_factor_corr[J] Lcorr_k;
...
transformed parameters{
  ...
  for(t in 1:L-1){
   g_dev[t,] = (diag_pre_multiply(rep_vector(sigma_g_s,J), Lcorr_g) * to_vector(z_dev_g[t,]))';
   k_dev[t,] = (diag_pre_multiply(rep_vector(sigma_k_s,J), Lcorr_k) * to_vector(z_dev_k[t,]))';
  }
...
model{
  ...
  Lcorr_g ~ lkj_corr_cholesky(1.0); // prior for correlation of process deviances
...
corr_matrix[J] Cor_J_g = multiply_lower_tri_self_transpose(Lcorr_g);
corr_matrix[J] Cor_J_k = multiply_lower_tri_self_transpose(Lcorr_k);
//vector[N] log_lik;
